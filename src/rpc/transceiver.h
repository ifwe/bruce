/* <rpc/transceiver.h>
   ...
   Call GetIoVecs() to get an array of sufficient size, then fill in the elements of
   the array to point to your data.  Then call Send() or Recv() repeatedly until
...
namespace Rpc {
  ...
  class TTransceiver final {
    ...
    /* True iff. we have data to transfer; that is, when we have at least one iovec with a non-empty buffer. */
    ...
    /* Scatter-read into the buffers currently in the iovec array.  The fd must be an open socket.
       Flags are as for recvmsg().  If there is an I/O error, or if our peer hangs up, we throw. */
    ...
    /* Initialze a msghdr structure to point at our array of initialized iovecs. */
    void InitHdr(msghdr &hdr) const noexcept;

    /* Takes the code returned by sendmsg() or recvmsg() and returns the actual number of bytes transferred.
       If the result indicates an error, or if no bytes were transferred, this function will throw appropriately. */
    static size_t GetActualIoSize(ssize_t io_result);

    /* The start and limit of our array of iovecs, or nulls if we haven't allocated the array yet.
       Always true: AvailStart <= AvailLimit. */
    iovec *AvailStart, *AvailLimit;

    /* The start and limit of the portion of the iovec array above which we have yet to transfer via I/O.
