...
A limitation of UNIX domain datagram sockets is the maximum size of
a single message.  In practice, this appears to not be a serious issue.  On a
CentOS 7 x86_64 installation, the maximum size has been experimentally
determined to be 212959 bytes.  This value increases to 425951 bytes if clients
are willing to increase `SO_SNDBUF` above the default value, although this may
be inconvenient for many clients.
